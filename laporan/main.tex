\documentclass[journal, a4paper]{IEEEtran}

% =================================================================
% PAKET-PAKET TEKNIS (ADVANCED)
% =================================================================
\usepackage[T1]{fontenc}      % Encoding font modern untuk kutipan lurus
\usepackage{lmodern}          % Menstandarisasi ketebalan font mesin tik agar kutipan tidak tebal
\usepackage{textcomp}         % Tambahan simbol
\usepackage{upquote}          % Kutipan lurus yang presisi di blok \verb dan \texttt
\usepackage[indonesian]{babel} % Menggunakan aturan tata bahasa Indonesia
\addto\captionsindonesian{
	\renewcommand{\abstractname}{Abstrak} % Mengubah 'Ringkasan' menjadi 'Abstrak'
	\renewcommand{\refname}{Referensi}    % Mengubah 'Pustaka' menjadi 'Referensi'
}
\usepackage[utf8]{inputenc}
\usepackage{graphicx} % Untuk menampilkan grafik dan flowchart
\usepackage{amsmath}  % Untuk penulisan rumus matematika kompleks
\usepackage{amssymb}
\usepackage{url}
\usepackage{cite}     % Manajemen sitasi standar IEEE
\usepackage{booktabs} % Untuk tabel profesional
\usepackage{float}    % Mengatur posisi gambar dan tabel secara presisi
\usepackage{hyperref} % Untuk tautan navigasi dalam dokumen
\usepackage{listings} % Untuk menampilkan lampiran kode program
\usepackage{xcolor}   % Untuk pewarnaan sintaks kode

% --- Konfigurasi Tampilan Kode Program ---
\definecolor{codegreen}{rgb}{0,0.6,0}
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{codepurple}{rgb}{0.58,0,0.82}
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}

\lstset{
	backgroundcolor=\color{backcolour},   
	commentstyle=\color{codegreen},
	keywordstyle=\color{magenta},
	numberstyle=\tiny\color{codegray},
	stringstyle=\color{codepurple},
	basicstyle=\ttfamily\footnotesize,
	breaklines=true,                 
	captionpos=b,                    
	keepspaces=true,                 
	numbers=left,                    
	numbersep=5pt,                  
	showspaces=false,                
	showstringspaces=false,
	showtabs=false,                  
	tabsize=2
}

% =================================================================
% IDENTITAS PENULIS DAN JUDUL
% =================================================================
\title{Analisis Komparatif Performa dan Keamanan Algoritma Hash SHA-256, SHA-3, dan BLAKE2 pada Lingkungan Python}

\author{
	Falito Eriano Nainggolan, Hinggil Parahita, Raffelino Hizkia Marbun, dan Yosapat Nainggolan\\
	\textit{Tingkat II Rekayasa Keamanan Siber A}\\
	\textit{Politeknik Siber dan Sandi Negara, Bogor, Indonesia}
}

\begin{document}
	
	\maketitle
	
	% =================================================================
	% RINGKASAN / ABSTRAK
	% =================================================================
	\begin{abstract}
		Studi ini mengevaluasi kinerja tingkat sistem fungsi hash SHA-256, SHA-3, dan BLAKE2 pada \textit{runtime} CPython (x86\_64, Windows 11). Evaluasi difokuskan pada \textit{throughput} komputasi, efisiensi alokasi CPU, dan kualitas difusi bit. Pengujian menggunakan korpus data 1 MB hingga 1 GB menunjukkan bahwa \textit{throughput} puncak SHA-256 (568 MB/s pada korpus 10 MB) melampaui BLAKE2 (247 MB/s) dan SHA-3 (175 MB/s). Analisis \textit{Two-Way Factorial ANOVA} mengonfirmasi efek interaksi Algoritma $\times$ Ukuran File yang signifikan, $F(6, 348) = 210,45, p < 0,001$ ($\eta_p^2 = 0,88$). Keunggulan SHA-256 berkorelasi langsung dengan \textit{offloading} instruksi \textit{Intel SHA Extensions} (SHA-NI); penonaktifan modul silikon ini menurunkan kinerjanya di bawah \textit{throughput} BLAKE2. SHA-256 mencatat efisiensi 1,8 \textit{CPU-second} per 1 GB, berbanding terbalik dengan SHA-3 (5,7 \textit{CPU-second} per 1 GB) akibat \textit{overhead} algoritma \textit{Sponge}. Pada pengujian \textit{Strict Avalanche Criterion} ($N=10.000$), ketiga fungsi menampilkan properti difusi yang konsisten dengan Distribusi Binomial via uji \textit{Chi-Square} ($p > 0,05$). Hasil eksperimen ini mengindikasikan bahwa kinerja algoritma kriptografis dalam bahasa tingkat tinggi lebih merefleksikan dukungan instruksi mikroarsitektur lokal dibandingkan profil kecepatan algoritmik murni.
	\end{abstract}
	
	\begin{IEEEkeywords}
		Kriptografi, Hash, SHA-256, SHA-3, BLAKE2, Benchmarking, Avalanche Effect, Intel SHA Extensions.
	\end{IEEEkeywords}
	
	% =================================================================
	% PEMANGGILAN FILE MODULAR (BAB 1 - 5)
	% =================================================================
	\input{bab1_pendahuluan}
	\input{bab2_tinjauan_pustaka}
	\input{bab3_metodologi}
	\input{bab4_hasil}
	\input{bab5_kesimpulan}
	
	% =================================================================
	% DAFTAR PUSTAKA
	% =================================================================
	\bibliographystyle{IEEEtran}
	\bibliography{referensi}
	
\end{document}